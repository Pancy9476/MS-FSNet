...
import os
from model_mamba_highlow import MultiScaleMambaFusion
...
def train(model, dataloader, criterion, optimizer, ema, device):
    ...
    for pan, ms, gt, _ in tqdm(dataloader, desc="Training"):
        ...
        total_loss = loss_main + 0.1 * loss_256 + 0.1 * loss_128 + 0.1 * loss_64 + color_loss + (1 - ssim(out, gt, data_range=1.0, size_average=True).to(device))
        total_loss.backward()
